...
@app.route("/cart/")
def show_shopping_cart(session):
    ...
    all_melons = melons.get_all()
    print('PRINT MELONS:', melons)

    total_cost = 0

    for melon_id in cart:
        numberof_this_kind_melon =  cart.get(melon_id)   
        price=melons.get_by_id(melon_id).price
        melon_cost = numberof_this_kind_melon * price 
    ...
    # melon_id = ...get(melon_id)
    
    # melon = melons.get_by_id(melon_id)

    return render_template("cart.html", total_cost = total_cost, melons=melons)
...
def add_to_cart(melon_id):
    ...
    print("cart is", cart)
    print(session)
    print("my session is ", session["cart"])
